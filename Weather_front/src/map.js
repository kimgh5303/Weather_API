...
const MapContainer = () => {
    ...
    const fetchLocations = async () => {
        try {
        const response = await axios.get('http://localhost:8080/weathers/locations');
    ...
    const addLocation = async () => {
        try {
            ...
            await axios.post('http://localhost:8080/weathers/locations', formData, {
                headers: {
    ...
    const updateLocation = async (id, address) => {
        try {
            ...
            await axios.put(`http://localhost:8080/weathers/locations`, {id, address}, {
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded', // x-www-form-urlencoded 형식으로 전송함을 명시
            },
            });
    ...
    const deleteLocation = async (locationId) => {
        try {
        await axios.delete(`http://localhost:8080/weathers/locations/${locationId}`);
        fetchLocations();
        } catch (error) {
